...
public class ClipZoomImageView extends ImageView implements
    ...
    public ClipZoomImageView(Context context, boolean isjiancai) {
        this(context, null);
        kspjiancai = isjiancai;
        if (isjiancai) {
            initScale = 1.0f;
        } else {
            initScale = 1.0f;
    ...
    public void onGlobalLayout() {
        if (once) {
            ...
            int boardHeight=getWidth();

            /*Log.e(TAG, "图片高度"+drawableH);
            Log.e(TAG, "图片宽度"+drawableW);
            Log.e(TAG, "框高度"+height);
            Log.e(TAG, "框宽度"+width);
            Log.e(TAG, "frameSize"+frameSize);*/
            ...
            float scaleW=1.0f*boardWidth/drawableW;
            float scaleH=1.0f*boardHeight/drawableH;
    ...
    public Bitmap clip(Bitmap kspBitmap, int kspwidth, int kspheight) {
        ...
        android.util.Log.e("这个照片高度", Integer.toString(bitmap.getHeight()));
        android.util.Log.e("这个照片宽度", Integer.toString(bitmap.getWidth()));
        ...
        if (bitmap != null) {
            ...
            try {
                ...
                if (kspjiancai) {
                    ...
                    if (kspBitmap.getHeight() == kspheight) {
                        return bitmap;
                    } else {
                        int bsss = bitmap.getWidth();
                        int dss = kspBitmap.getWidth();
                        int height = bitmap.getHeight();
                        Double huilv = (double) bsss / (double) dss;
                        android.util.Log.e("计算", "" + huilv);
                        int heig = kspBitmap.getHeight();
                        int realHeight = (int) (huilv * heig);
                        android.util.Log.e("计算", "" + realHeight);
                        int chazhi = (height - realHeight) / 2;
    ...
    private class AutoScaleRunnable implements Runnable {
        ...
        static final float SMALLER = 0.5f;
